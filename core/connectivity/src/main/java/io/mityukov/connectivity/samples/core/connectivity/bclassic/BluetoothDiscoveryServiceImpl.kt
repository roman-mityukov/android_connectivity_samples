...
class BluetoothDiscoveryServiceImpl @Inject constructor(
    ...
    private val mutableStateFlow = MutableStateFlow(
        BluetoothDiscoveryState(
            progress = BluetoothDiscoveryProgress.Finished,
            discoveredDevices = setOf(),
        )
    )

    override val discoveryFlow = mutableStateFlow.asStateFlow()
    ...
    override suspend fun startDiscovery(): StartDiscoveryResult = withContext(dispatcher) {
        this@BluetoothDiscoveryServiceImpl.logd("startDiscovery")
        when (val status = statusService.status) {
            BluetoothStatus.Ok -> {
                ...
                StartDiscoveryResult.Success
            }

            else -> {
                StartDiscoveryResult.Failure(status)
            }
        }
    }
    ...
    override fun stopDiscovery() {
        ...
        coroutineScope.launch {
            val latestState = discoveryFlow.first()
            mutableStateFlow.update {
                latestState.copy(
                    progress = BluetoothDiscoveryProgress.Finished,
                    discoveredDevices = latestState.discoveredDevices
                )
            }
        }
    }
            ...
            when (action) {
                BluetoothAdapter.ACTION_DISCOVERY_STARTED -> {
                    ...
                    coroutineScope.launch {
                        val currentState = discoveryFlow.first()
                        mutableStateFlow.update {
                            currentState.copy(progress = BluetoothDiscoveryProgress.Started)
                        }
                    }
                }
                ...
                BluetoothAdapter.ACTION_DISCOVERY_FINISHED -> {
                    ...
                    coroutineScope.launch {
                        val currentState = discoveryFlow.first()
                        mutableStateFlow.update {
                            currentState.copy(progress = BluetoothDiscoveryProgress.Finished)
                        }
                    }
                }
                ...
                BluetoothDevice.ACTION_FOUND -> {
                    ...
                    coroutineScope.launch {
                        ...
                        val newDiscoveredDevices = mutableSetOf<DiscoveredDevice>()
                        newDiscoveredDevices.addAll(currentState.discoveredDevices)
                        newDiscoveredDevices.add(
                            DiscoveredDevice(
                                name = device.name,
